...
# for tensorflow2
from memoryrf import MemoryDNN
...
if __name__ == "__main__":
    ...
    K = N                   # initialize K = N
    decoder_mode = 'RF'    # the quantization mode could be 'OP' (Order-preserving) or 'RF'
    ...
    rate = sio.loadmat('data_%d' %N)['output_obj'] # this rate is only used to plot figures; never used to train DROO.
    ...
    for i in range(n):
        ...
        # the action selection must be either 'OP' or 'RF'
        ...
        K_his.append(K)
        # print("mode_his"+m_list[np.argmax(r_list)])
